//
//  BodyTypeSelectionViewController.swift
//  TopCariving
//
...
class BodyTypeSelectionViewController: TrimSelectionViewController {
    // MARK: - UI properties
    
    // MARK: - Properties
    let viewModel: BodyTypeSelectionViewModel
    
    // MARK: - Lifecycles
    init(viewModel: BodyTypeSelectionViewModel) {
    ...
    private func bind() {
        let output = viewModel.transform(input: .init(
            ...
            tapNextButtonPublisher: footerView.tapNextButton.throttle(for: 1.0, scheduler: DispatchQueue.main, latest: false).eraseToAnyPublisher(),
            tapBodyTypeIndexPublisher: foldableStackView.tapSubject.eraseToAnyPublisher()
        ))
        output.errorSubject
        ...
        output.pushSubject
            ...
            .sink(receiveValue: { [weak self] archivingID in
                ...
                let viewModel = DrivingMethodSelectionViewModel(httpClient: httpClient,
                                                                archivingID: archivingID.data)
                let viewController = DrivingMethodSelectionViewController(viewModel: viewModel)
